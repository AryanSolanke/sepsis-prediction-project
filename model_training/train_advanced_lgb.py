...
def main():
    ...
    # Converting to np.float32 for efficiency
    ...
    lgb = LGBMClassifier(
        ...
        random_state=42,
        n_jobs=-1 # LightGBM will use all cores for each individual fit
    )
    ...
    # NOTE: Set n_jobs=1 because LGBMClassifier already uses n_jobs=-1 internally.
    ...
    # Suppress verbose LightGBM warnings during grid search
    ...
    eval_dir = f"Performance_eval/LightGBM"
    os.makedirs(eval_dir, exist_ok=True)
    
    # 1. ROC Curve
    fig, ax = plt.subplots(figsize=(8, 6))
    RocCurveDisplay.from_predictions(y_val, y_proba, ax=ax)
    plt.title(f"LightGBM - ROC Curve")
    ...
    plt.title(f"LightGBM - Precision-Recall Curve")
    ...
    plt.title(f"LightGBM - Calibration Plot")
    ...
    plt.title(f"LightGBM - Confusion Matrix (Threshold: {best_t:.2f})")
    ...
    if lead_times:
        ...
        plt.title("LightGBM Lead Time Distribution")
    ...
    if hasattr(best_lgb, 'feature_importances_'):
        ...
        plt.title(f"LightGBM - Top 20 Feature Importances")
        plt.tight_layout()
    ...
    with open('model_reports.txt', 'a') as f:
        f.write("\n=== ADVANCED LightGBM (Comprehensive Evaluation) ===\n")
