...
from deep_learning_common import (
    ...
    set_seed,
    train_model,
    transform_tabular_frame,
)


class SepsisFeedForwardNet(nn.Module):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a deep feedforward neural network for sepsis prediction.")
...
def build_dataloaders(split_frames: SplitFrames, batch_size: int, balanced_sampling: bool):
    scaler = build_scaler(split_frames.train, split_frames.feature_names)

    X_train, y_train = transform_tabular_frame(split_frames.train, scaler, split_frames.feature_names)
    X_val, y_val = transform_tabular_frame(split_frames.val, scaler, split_frames.feature_names)
    X_test, y_test = transform_tabular_frame(split_frames.test, scaler, split_frames.feature_names)
...
def main() -> None:
    ...
    print_final_report("Deep Neural Network", test_metrics, baseline_metrics=baseline_test_metrics)
